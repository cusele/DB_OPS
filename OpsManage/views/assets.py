...
def getBaseAssets():
    ...
    try:
        ...
        projectList = []          
    return {"group":groupList,"service":serviceList,"zone":zoneList,'project':projectList}
...
def assets_dumps(request):
    if request.method == "POST":  
        ...
        bList = ['设备类型','资产编号','设备序列号','购买时间','过保时间','购买人','管理IP','生产制造商','设备型号','供货商',
                 ...
                 '物理CPU','逻辑CPU','CPU核心数','内存容量','内核版本','Selinux状态','Swap分区','磁盘空间',
                 '系统版本号']
        ...
        for ast in request.POST.get('assetsIds').split(','):
            ...
            if assets.assets_type in ['vmser','server']:
                ...
                sheet.write(count,21,assets.server_assets.cpu_number,dRbt.bodySttle())
                sheet.write(count,22,assets.server_assets.vcpu_number,dRbt.bodySttle())
                sheet.write(count,23,assets.server_assets.cpu_core,dRbt.bodySttle())
                sheet.write(count,24,assets.server_assets.ram_total,dRbt.bodySttle())
                sheet.write(count,25,assets.server_assets.kernel,dRbt.bodySttle())
                sheet.write(count,26,assets.server_assets.selinux,dRbt.bodySttle())
                sheet.write(count,27,assets.server_assets.swap,dRbt.bodySttle())
                sheet.write(count,28,assets.server_assets.disk_total,dRbt.bodySttle())
                sheet.write(count,29,assets.server_assets.system,dRbt.bodySttle())
            else:
                sheet = netSheet
